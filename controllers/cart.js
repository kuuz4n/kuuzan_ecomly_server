...
exports.addToCart = async function (req, res) {
  ...
  try {
    ...
    if (!cartProduct) {
      ...
      return res
        .status(500)
        .json({ message: "The product could not be added to the cart" });
    }
...
exports.modifyProductQuantity = async function (req, res) {
  try {
    ...
    if (quantity > actualProduct.countInStock) {
      return res
        .status(400)
        .json({ message: "Insufficient stock of this product" });
    }
...
exports.removeFromCart = async function (req, res) {
  ...
  try {
    ...
    if (!user.cart.includes(req.params.cartProductId)) {
      await session.abortTransaction();
      return res.status(400).json({ mesasge: "Product not in your cart!!" });
    }
    ...
    if (!cartProduct) {
      return res.status(500).son({ message: "Internal Server Error" });
    }
    await session.commitTransaction();
